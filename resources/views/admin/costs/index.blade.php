...
<body class="bg-[#f9fafb] min-h-screen">
    ...
    <div class="max-w-7xl mx-auto px-8 py-6">
        <!-- Header -->
        <div class="mb-6">
            <h1 class="text-xl font-semibold text-gray-900">Transcription Costs</h1>
            <p class="text-sm text-gray-500">Monitor Deepgram usage and expenses</p>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4 mb-6">
            <form method="GET" action="{{ route('admin.costs.index') }}" class="flex gap-4 items-end">
                <div>
                    ...
                    <input type="date" name="date_from" value="{{ $filters['date_from'] }}" class="border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
                </div>
                <div>
                    <label class="block text-sm text-gray-500 mb-1">To</label>
                    <input type="date" name="date_to" value="{{ $filters['date_to'] }}" class="border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
                </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
                ...
                <p class="text-2xl font-semibold text-gray-900">${{ number_format($overallStats['total_cost'], 2) }}</p>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
                ...
                <p class="text-2xl font-semibold text-blue-600">{{ number_format($overallStats['total_transcriptions']) }}</p>
            </div>
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
                <p class="text-sm text-gray-500">Total Minutes</p>
                <p class="text-2xl font-semibold text-gray-900">{{ number_format($overallStats['total_minutes'], 1) }}</p>
            </div>
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
                <p class="text-sm text-gray-500">Avg Cost/Call</p>
                <p class="text-2xl font-semibold text-gray-900">${{ number_format($overallStats['avg_cost_per_call'], 3) }}</p>
            </div>
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
                <p class="text-sm text-gray-500">Cost/Minute</p>
                <p class="text-2xl font-semibold text-gray-900">${{ number_format($overallStats['cost_per_minute'], 4) }}</p>
            </div>
        </div>
